...
class ModelMetaclass(type):
	...
	def __new__(cls, name, bases, attrs):
		...
		#attrs是一个字典，包含类具有的属性和方法  例如：类中包含 id = IntegerField('12')  attrs中就包含attrs['id'] = IntegerField('12')
...
class Model(dict, metaclass=ModelMetaclass): 
	...
	def getValueOrDefault(self, key):
		...
		if value is None:
			...
			if field.default is not None:
				...
				logging.debug('using default value : %s for %s ' % (str(value), key))
	...
	async def findNumber(cls, selectfield, where=None, args=None):
		' select number by select an where '
		sql = ['select %s a from `%s`' % (selectfield, cls.__table__)]
		if where:
			sql.append('where')
			sql.append(where)
